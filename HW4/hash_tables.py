...
class HashTable:
    ...
    def two_choice_hashing(self, occupancy_at_slot):
        ...
        # Return the final slot number that you pick.
        slot = 0
        c1 = random.randint(0, self.num_slots-1)
        c2 = random.randint(0, self.num_slots-1)
        if occupancy_at_slot[c1] < occupancy_at_slot[c2]:
            slot = c1
        elif occupancy_at_slot[c1] > occupancy_at_slot[c2]:
            slot = c2
        else:
    ...
    def two_left_hashing(self, occupancy_at_slot):
        ...
        # Return the final slot number that you pick.
        slot = 0
        c1 = random.randint(0, self.num_slots//2-1)
        c2 = random.randint(self.num_slots//2, self.num_slots-1)
#        print (self.num_slots//2-1)
