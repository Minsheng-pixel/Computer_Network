...
class DVRouter(Router):
    ...
    def process_advertisement(self, dv_adv, adv_router):
        ...
        for item in self.dv.keys():
            if item in dv_adv.keys():
                if dv_adv[item] + self.links[adv_router] < self.dv[item]:
                    self.dv[item] = dv_adv[item] + self.links[adv_router]
                    self.dv_change = True
                    self.fwd_table[item] = adv_router
        for item in dv_adv.keys():
            if item not in self.dv.keys():
                self.dv[item] = dv_adv[item] + self.links[adv_router]
                self.dv_change = True
                self.fwd_table[item] = adv_router
