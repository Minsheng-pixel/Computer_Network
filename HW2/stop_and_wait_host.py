...
#           self.in_order_rx_seq = self.in_order_rx_seq + 1
    ...
    def recv(self, pkt, tick):
        ...
        # TODO: Update self.in_order_rx_seq and self.ready_to_send depending on pkt.seq_num// DO I need to check for seq/
        if pkt.seq_num-self.in_order_rx_seq == 1:
            self.in_order_rx_seq = pkt.seq_num
            self.ready_to_send = True
        # TODO: Only print this when you received the packet
        self.verbose = self.ready_to_send
        if self.verbose:
            print(
                "rx packet @ "
                + str(tick)
