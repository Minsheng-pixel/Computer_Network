...
class FifoSimulator:
    ...
    def fifo(self, tick):
        ...
        d = {}
        for index, val in enumerate(self.input_queues):
            if len(val) != 0:
                output_ports = val[0].output_port
            else:
                continue
            if output_ports in d:
                d[output_ports].append(val[0])
            else:
                d[output_ports] = [val[0]]
        for i in d:
            picked = random.choice(d[i])
            self.input_queues[picked.input_port].pop(0)
            self.delay_count += 1
